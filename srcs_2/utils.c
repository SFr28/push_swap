...
/*   Updated: 2025/01/23 12:40:16 by sfraslin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int ft_choose(t_pile **pile_a, t_pile **pile_b, t_pile *node)
{
    ...
    // one = node->target->position + size_b - node->position;
    // two = node->target->position + node->position;
    // three = size_a - node->target->position + node->position;
    // four = size_a - node->target->position + size_b - node->position;
    if ((node->position <= size_b / 2 && node->target->position <= size_a / 2)
            || (node->position > size_b / 2 && node->target->position > size_a / 2))
        return (1);
